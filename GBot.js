// ============================================
// 🚀 GBot Instalador V2 - By Nz
// COM VERIFICAÇÃO DE SERIAL E DEVICE
// ============================================
...
var GITHUB_TOKEN = "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";
...
var URL_FINALIZADOR = "https://raw.githubusercontent.com/" + OWNER + "/" + REPO + "/main/Finalizador.js";
...
var logs = [];
var serial = null;
var serialOriginal = null;
var autorizado = false;
var dadosUsuario = null;
var deviceRegistrado = false;
...
function addLog(msg, tipo) {
    ...
    else if (tipo === 'serial') icon = "[SERIAL]";
    else if (tipo === 'device') icon = "[DEVICE]";
    else if (tipo === 'github') icon = "[GITHUB]";
    
    var entry = "[" + d + "] " + icon + " " + msg + "\n";
    logs.push(entry);
    if (logs.length > 300) logs.splice(0, 50);
    ...
    try {
        ...
        if (v) {
            var txt = v.text();
            var lines = txt.split('\n');
            if (lines.length > 300) lines.splice(0, 50);
            lines.push(entry);
            v.setText(lines.join('\n'));
            ui.logScroll.scrollTo(0, v.getHeight());
            ui.logCount.setText(logs.length.toString());
        }
    } catch(e) {}
}

function updateStatus(texto, cor) {
    try {
        ui.statusText.setText(texto);
        if (cor) {
            ui.statusText.setTextColor(colors.parseColor(cor));
        }
    } catch(e) {}
}
...
function httpRequest(url, method, data, headers, cb) {
